 ...
 */
class WidthIterator<K extends Comparable<K>, T extends AbstractStructureElement & ISplayData<K>> implements Iterator<ExtendData<K,T>> {
    private LinkedList<IteratorHelper> fifo = new LinkedList<>();

    private IteratorHelper temp = null;

    WidthIterator(SplayNode<K,T> root,boolean isLeft){
        if (root==null)return;
    ...
    private class IteratorHelper {
        ...
        private IteratorHelper(SplayNode<K,T> node, boolean isLeft) {
            this.node = node;
            this.isLeft = isLeft;
        }
    }
